...
impl IsDao for ApiKey{
    ...
    fn to_dao(&self)->Dao{
        ...
        dao.set("api_key_id", &self.api_key_id);
        dao.set("api_key", &self.api_key);
        dao.set("user_id", &self.user_id);
        match self.valid_starting{
            Some(ref _value) => dao.set("valid_starting", _value),
            None => dao.set_null("valid_starting")
        }
        match self.valid_until{
            Some(ref _value) => dao.set("valid_until", _value),
            None => dao.set_null("valid_until")
        }
        dao
    }
}
